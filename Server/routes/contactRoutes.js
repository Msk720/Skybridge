/**
 * Module: Routes
 * Purpose: Handles contact messages and replies.
 */
...
function normalizeMessage(doc, source = "contactmessages", parent = null) {
...
router.post("/", protect, async (req, res) => {
  try {
    ...
    const isNewChat = !(await ContactMessage.exists(
    ...
    return res.status(201).json(normalizeMessage(savedMessage, "contactmessages"));
  } catch (error) {
...
router.get("/my", protect, async (req, res) => {
  try {
    ...
    const contactMessages = await ContactMessage.find(contactQuery)
      .sort({ createdAt: -1, _id: -1 })
      .lean();

    const messages = contactMessages
      .map((item) => normalizeMessage(item, "contactmessages"))
      .filter((item) => item.message)
      .sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));

    return res.json(messages);
  } catch (error) {
    console.error("My contact messages error:", error.message);
    return res.status(500).json({ message: "Server error" });
  }
});

// @route   GET /api/contact/messages
// @desc    Get all contact messages for the platform dashboard
// @access  Protected platform
router.get("/messages", protect, requireRole, async (req, res) => {
  try {
    ...
    console.error("Contact messages error:", error.message);
    return res.status(500).json({ message: "Server error" });
  }
});
...
// @access  Protected platform
router.post("/messages/:id/reply", protect, requireRole, async (req, res) => {
  try {
    const imageUrl = req.body.imageUrl?.trim() || req.body.attachmentUrl?.trim() || req.body.fileUrl?.trim() || "";
    ...
    let updated = await ContactMessage.findByIdAndUpdate(
      req.params.id,
      {
    ...
    if (updated) {
      ...
      return res.json(normalizeMessage(updated, "contactmessages"));
    }
